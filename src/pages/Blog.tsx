...
const Blog = () => {
      ...
      try {
        ...
        const techPosts = stories
          ...
          .map((story) => ({
            ...
            featured: false, // This is not used anymore, filtering is date-based
  ...
  const getRealPerplexityContent = (): BlogPost[] => {
    ...
    const formatDate = (timestamp: number) => 
      language === 'hu' 
        ? new Date(timestamp).toLocaleDateString('hu-HU', { year: 'numeric', month: 'long', day: 'numeric' })
  ...
  const featuredPosts = filteredPosts.filter(p => new Date(p.date).toDateString() === todayStr);
  
  const allOtherPosts = filteredPosts.filter(p => {
    const postDate = new Date(p.date);
    return postDate.toDateString() !== todayStr && postDate >= twoWeeksAgo;
  });
                      ...
                      <CardHeader>
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          <div className="flex items-center text-sm text-muted-foreground">
                            ...
                            {new Date(post.date).toLocaleDateString(language === 'hu' ? 'hu-HU' : 'en-US')}
                          </div>
                          <div className="flex items-center text-sm text-muted-foreground">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-4 text-sm text-muted-foreground mb-4">
                    <div className="flex items-center gap-1">
                      ...
                      {new Date(selectedPost.date).toLocaleDateString(language === 'hu' ? 'hu-HU' : 'en-US')}
                    </div>
                    <div className="flex items-center gap-1">
